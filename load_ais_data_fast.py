#!/usr/bin/env python3
"""
Optimized AIS Data Loader using batch inserts and executemany().
Usage: python load_ais_data_fast.py
"""
...
def load_csv_to_database(connection):
    """Load CSV data into the database using batch inserts."""
    ...
    try:
        ...
        print(f"Found {len(df_test.columns)} columns")
        ...
        for chunk_idx, df in enumerate(pd.read_csv(CSV_FILE, chunksize=BATCH_SIZE)):
            ...
            if len(batch_data) >= BATCH_SIZE:
                with closing(connection.cursor()) as cursor:
                    cursor.executemany(
                        f"INSERT INTO {SOURCE_TABLE} (timestamp, mmsi, lat, lon) VALUES (?, ?, ?, ?)",
                        batch_data
                    )
                connection.commit()
...
def main():
    """Main execution."""
    print("\n" + "=" * 60)
    print("AIS Data Loader (Optimized)")
    print("=" * 60)
    ...
    try:
        ...
        if existing_rows > 0:
            print(f"⚠ Table {SOURCE_TABLE} already has {existing_rows:,} rows")
            ...
            if response == "y":
                ...
                print("Skipping CSV load, rebuilding lookup table...")
