...
This isolates ONLY the rows removed by removeOutliers.remove_gps_outliers().
...
Reports:
  ...
  - Deep dive on top affected ships: shows sample deleted rows with SOG,
    distance to prev/next, expected distance, and why the forward_pass
    flagged them

Usage:
    python validate_cleaning.py
    python validate_cleaning.py --input AISDATA/other.csv --top 50 --deep 5
"""
...
def run_pipeline_before_outliers(spark, input_path):
    """Run every pipeline step from main.py BEFORE removeOutliers."""
    df = (
...
def tag_deleted_rows(before_df, after_df):
    ...
    Return the before_df with extra columns:
      ...
      _exp_km     : expected km from forward_pass formula
      _margin     : margin used by forward_pass
    """
    after_keys = (
...
def main():
    ...
    after_outliers = removeOutliers.remove_gps_outliers(before_outliers)
