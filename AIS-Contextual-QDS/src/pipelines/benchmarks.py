...
def _parse_methods(methods: list[str] | None, config: AppConfig) -> list[str]:
    raw_methods = methods or config.benchmarks.methods
...
def run(
    ...
    *,
    ...
    selected_split = split or config.benchmarks.default_split
        ...
        for budget in selected_budgets:
            ...
            for trajectory_id, points in trajectories.items():
                ...
                kept_indices = _simplify_indices(
                    ...
                    points,
                    target_points,
                    dp_search_iterations=config.benchmarks.dp_search_iterations,
                ...
                if len(buffer) >= config.benchmarks.insert_batch_size:
