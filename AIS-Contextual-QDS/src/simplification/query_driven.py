"""Query-driven, context-unaware simplification for B3."""
...
class B3PointEvidence:
    """Trajectory-local query and shape evidence available to the B3 scorer."""
...
@dataclass(frozen=True)
class B3ScoreComponents:
    """Named score components used for deterministic B3 ranking."""
    ...
    def total(self) -> float:
        """Weighted B3 score. Endpoints are handled as forced anchors."""
        return (
...
def b3_score_components(evidence: B3PointEvidence, *, max_local_deviation_m: float) -> B3ScoreComponents:
    """Compute B3 score components for one non-anchor point."""
...
def score_b3_points(evidence: list[B3PointEvidence]) -> list[float]:
    """Return scalar B3 scores for all points, with endpoints marked as anchors."""
    if not evidence:
        return []
    ...
    for idx, item in enumerate(evidence):
        if idx == 0 or idx == len(evidence) - 1:
            ...
            continue
        scores.append(b3_score_components(item, max_local_deviation_m=max_deviation).total)
    return scores


def simplify_b3_indices(evidence: list[B3PointEvidence], target_points: int) -> list[int]:
    """Keep highest-scoring B3 points while preserving first and last point."""
    ...
    for idx, item in enumerate(evidence[1:-1], start=1):
        components = b3_score_components(item, max_local_deviation_m=max_deviation)
